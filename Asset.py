...
class Asset: #Classe instancié chaque heure, qui gardera donc la valeur en €, $ et BTC historique
    def __init__(self, name: str, type: AssetType, source: AssetSource, timestamp: datetime, currency_exprimed: str, usd_eur: float, btc_eur: float, btc_usd: float, amount: float = None, price: float = None, value: float = None, ticker: str = None, isin: str = None, wallet_name: str = None, defi_type: str = None):
        self.name = name
        ...
        if price != None:
            if currency_exprimed == 'EUR':
                ...
                raise ValueError(f"{currency_exprimed} n'est pas une devise prise en charge pour l'Asset")
        else:
